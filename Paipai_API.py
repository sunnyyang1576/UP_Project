...
class PaiPai_HF_API:
    ...
    def get_create_time(self):
        '''
        This function returns the creation time of a fund.
        ...
        try:
            ...
            cur.execute("SELECT fund_id as fund_id,\
            CONVERT(REPLACE(updatetime,'-','') ,SIGNED) AS updatetime,\
                    createtime FROM rz_hfdb_core.pvn_fund_attributes \
                    WHERE fund_id IN " + str(tuple(self._fund_id_list)) + ";")

            data = cur.fetchall()
            df = pd.DataFrame(data, index=None, columns=['fund_id', 'updatetime', 'createtime'])
            return df
        except Exception as e:
            print(e)
        finally:
